...
typedef struct
{
    ...
    unsigned int    pad1;
} __attribute__((packed)) irq_descriptor_t;

typedef struct
{
    unsigned short  size;
    unsigned long   addr;
} __attribute__((packed)) idt_selector_t;

static irq_descriptor_t g_idt[256];

static void setup_pic()
{
  debug(IRQ, "setting up interrupt controller\n");
...
static void setup_idt()
{
  ...
  idtSelector.size = sizeof(irq_descriptor_t) * 256 - 1;
  idtSelector.addr = (unsigned long)g_idt;
